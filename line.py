class Line:
    ...
    def hasnextconfiguration(self):
        """
        Whether the configuation of the given line can be incremented. False if
        the end position has been reached by all segments.
        """
        pos = self.length
        for seg in reversed(self.segments):
            pos -= seg.lengthfull
            if seg.pos != pos:
                #Then segment is not at its rightmost position
                return True
        return False

    def setnextconfiguration(self):
        assert self.hasnextconfiguration(), 'No more configurations'
        ...
        while not done:
            i_seg -= 1
            assert i_seg >= 0 #should not happen, due to the assert at the top
            if self.issegmentincrementable(i_seg):
                done = True
    ...
    #Methods used by "slave lines":
    def isuncoveredconfvalid(self, uncoveredconf):
        pos = 0
        for seg in self.segments:
            done = False
            while not done:
                if pos + seg.lengthfull > self.length: #using getposaftersegment doesn't work since segments aren't actually moved
                    return False
                if seg.matchespotentially(uncoveredconf[pos:pos+seg.lengthfull]):
                    pos += seg.lengthfull #skip over current segment...
                    done = True #...and go to next segment
                else:
                    if uncoveredconf[pos] > 0: #non-empty block passed
                        return False
                    pos += 1
    ...
    class Segment:
        ...
        def matches(self, conf):
            ...
            for i in range(self.length):
                if conf[i] != self.getcoloridxat(i):
                    return False
            return True
        ...
        def matchespotentially(self, conf):
            ...
            for i in range(self.lengthfull):
                if conf[i] >= 0 and conf[i] != self.getcoloridxat(i):
                    return False
            return True